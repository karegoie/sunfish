...
void generate_morlet_wavelet(double scale, int length, cplx* output) {
  // Morlet wavelet: ψ(t) = (1/√(s·π^(1/4))) * exp(-1/2 * (t/s)^2) * exp(-j*2π*t/s)
...
bool compute_cwt_features(const char* sequence, int seq_len,
  ...
  for (int s = 0; s < num_scales; s++) {
    ...
    if (wavelet_len < 10) wavelet_len = 10;
    if (wavelet_len > seq_len) wavelet_len = seq_len;
    ...
    if (!convolve_with_wavelet(signal, seq_len, wavelet, wavelet_len, features[s])) {
      free(wavelet);
      free(signal);
      return false;
    }
