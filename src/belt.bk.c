...
void train_lasso_logistic_regression(const double *const *X, const int *y, int n_samples, int n_features,
                                     double *out_coeffs, double learning_rate, int iterations, double lambda)
{
...
bool parse_args(int argc, char *argv[], CommandLineArgs *args)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (strcmp(argv[i], "--goi") == 0 && i + 1 < argc)
        {
            ...
            while (token)
            {
                args->goi_list = (char **)realloc(args->goi_list, sizeof(char *) * (args->goi_count + 1));
    ...
    if (!args->csv_path || !args->output_prefix || (!args->all_genes && args->goi_count == 0))
    {
        fprintf(stderr, "Usage: %s --csv <path> --output <prefix> [--all | --goi <g1,g2,...>] [--threshold <float>]\n", argv[0]);
        return false;
    }
...
bool read_csv(const char *path, DataTable *table)
{
    ...
    while (fgets(line, sizeof(line), file) && gene_idx < table->num_genes)
    {
        ...
        int sample_idx = 0;
        while ((token = strtok(NULL, ",")) != NULL && sample_idx < table->num_samples)
        {
...
void *worker_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        train_lasso_logistic_regression((const double *const *)X, y, data->table->num_samples, n_features, coeffs, 0.01, 1000, 0.1);
        ...
        for (int g = 0; g < data->table->num_genes; ++g)
        {
            if (g == goi_idx)
                continue;
            fprintf(data->beta_file, "%s,%s,%.6f\n", goi_id, data->table->gene_ids[g], coeffs[feature_idx + 1]);
            feature_idx++;
        }
        ...
        for (int s = 0; s < data->table->num_samples; ++s)
        {
            ...
            fprintf(data->prob_file, "%s,%s,%d,%.6f\n", goi_id, data->table->sample_ids[s], y[s], prob);
        }
...
void run_analysis(CommandLineArgs *args, DataTable *table)
{
    ...
    fprintf(prob_file, "Analyzed_GOI_ID,Sample_ID,Actual_Expression_Class,Predicted_Probability_High\n");
    ...
    for (long i = 0; i < num_cores; ++i)
    {
        thread_data[i] = (ThreadData){
            .thread_id = (int)i, .args = args, .table = table, .analysis_list = analysis_list, .analysis_count = analysis_count, .next_job_index = &next_job_index, .job_mutex = &job_mutex, .prob_file_mutex = &prob_file_mutex, .beta_file_mutex = &beta_file_mutex, .prob_file = prob_file, .beta_file = beta_file};
